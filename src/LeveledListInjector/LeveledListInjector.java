...
//        }
    ...
    public void theInitFunction() throws IOException {
        ...
        for (ARMO armor : gearVariants.getArmors()) {
            ...
            keywordChecker(armor.getForm(), keys, armor.getEDID());
        }
        for (WEAP weapon : gearVariants.getWeapons()) {
            ...
            keywordChecker(weapon.getForm(), keys, weapon.getEDID());
        }
    ...
    private void keywordChecker(FormID form, KeywordSet keys, String edid) {
        for (FormID kw : keys.getKeywordRefs()) {
            KYWD key = (KYWD) gearVariants.getMajor(kw, GRUP_TYPE.KYWD);
            if (key == null) {
                String error = edid
                        + " has an invalid keyword reference: "+ kw
                        + "\nThe patch will fail. Clean it in tes5edit and rerun the patcher.\n" +
                        "\n" + form.getFormStr() + " " + form.getTitle() + " " + form.getMaster().print();
                SPGlobal.logError("Keyword", error);
                //throw new RuntimeException(error); 
            }
        }
    }
